...
/* 链表删除指定元素 */
int LinkListAppointDataDel(LinkList * pList, ELEMENTTYPE val)
{
    int ret = 0;

    int pos = 0;
    int size = 0;
    while (LinkListLenghth(pList, &size) && pos != NOT_FIND)
    {
        int pos = 0;
        LinkListAccordAppointValGetPos(pList, val, &pos);
        LinkListAppointPosDel(pList, pos);
    }
...
int LinkListDestory(LinkList * pList)
{
    ...
    /* 运用头删 进行循环的删除 */
    int size = 0;
    while (LinkListLenghth(pList, &size))
    {
